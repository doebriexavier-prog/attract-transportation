...
function normalizeRequest(booking) {
  return {
    ...
    email: booking.Email || '',
    phone: booking.phone || '',
    pickup: booking['pick up'] || booking.pickup || '',
    destination: booking.destinatio || booking.destination || '',
    date: booking.Date || booking.date || '',
    time: booking.Time || booking.time || '',
    passengers: Number(booking.Passangers ?? booking.passengers ?? 0),
    ...
    customServiceType: booking.custom_service_type || '',
    customService: booking.custom_service_type || '',
    airport_airline: booking.airport_airline || '',
    airport_flight_number: booking.airport_flight_number || '',
    airport_baggage_amount: booking.airport_baggage_amount || '',
    has_baggage: booking.has_baggage || '',
    baggage_type: booking.baggage_type || '',
    baggage_amount: booking.baggage_amount || '',
    baggage_items: booking.baggage_items || '',
